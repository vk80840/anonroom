...
import { ArrowLeft, Moon, Sun, Check, User, Lock, Shield, Palette, Camera, Plus, X, Users, Keyboard, Image, Sparkles, Bell, BellOff, ExternalLink, Unlink } from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const { 
    ...
    revokeConnection: revokeTelegram,
    checkConnection: refreshTelegram,
            ...
            <div className="bg-card border border-border rounded-xl p-6 relative overflow-hidden">
              ...
              {telegramLoading ? (
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {telegramConnection?.telegram_username && (
                          <p className="text-xs text-muted-foreground">@{telegramConnection.telegram_username}</p>
                        )}
                      </div>
                    </div>
                    <Button 
                      ...
                      size="sm"
                      onClick={async () => {
                        const success = await revokeTelegram();
                        if (success) {
                  ...
                  <p className="text-xs text-muted-foreground">
                    Connected since {new Date(telegramConnection?.connected_at || '').toLocaleDateString()}
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Get notifications via Telegram even when the app is closed. Connect your account to receive message alerts.
                  </p>
                  <a
                  ...
                  >
                    <Button className="gap-2 bg-blue-500 hover:bg-blue-600">
                      ...
                      Connect via @Anonroom_robot
                    </Button>
                  </a>
                  <p className="text-xs text-muted-foreground">
                    1. Click the button above<br/>
                    2. Start the bot and use /login with your credentials<br/>
                    3. Come back here and refresh to see your connection
                  </p>
                  <Button variant="outline" size="sm" onClick={refreshTelegram}>
                    Refresh Status
                  </Button>
                </div>
              )}
